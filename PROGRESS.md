...
- learning: F8 denylist UI (#60) is presentation-only — settings.js persists only the USER array; DEFAULT_DENYLIST in field-qualify.js is the unconditional R3 floor merged at qualify time. So the UI shows defaults read-only; making a default *removable* would need a floor/override logic change (new storage + field-qualify tweak), out of scope for a presentation-only card. "Takes effect on next focus" is free: the ghost-icon follower reads the live getSettings() cache each focus event.
- [2026-07-19T07:56:06Z] iter 1/10 → 017967f  cost=$2.4026175000000007 duration=510834ms
