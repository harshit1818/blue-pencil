...
# back to its own stop-for-human path. Both loop.sh's gates and the build agent
# (via Bash, mid-iteration) use this — it's a plain shell command, so it works
# in a headless `claude -p` run where the Slack MCP would not.
#
#   ans="$(scripts/ask-human.sh "Merge #58 despite the flaky test?" "yes" "no")" \
#     && echo "human chose: $ans" || echo "no answer — stopping for a human"
#
# Reply in the Slack THREAD with a number (1-based) or free text.
# Env: SLACK_BOT_TOKEN, SLACK_CHANNEL (required); ASK_POLL=20, ASK_TIMEOUT=1800.
